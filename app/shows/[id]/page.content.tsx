            ...
            <AccordionItem value={key} key={key} className="border-none py-0">
              <AccordionTrigger className="hover:no-underline py-4">
                <div className="flex items-center justify-start w-full gap-1 hover:no-underline">
                  <span className="text-2xl font-semibold">{SOURCE_PROVIDER[key as keyof typeof SOURCE_PROVIDER]}</span>
                  <span className="text-2xl font-light">({value?.length})</span>
                </div>
              </AccordionTrigger>
              <AccordionContent>
                <div className="flex flex-col gap-6 pt-6">
                  {value?.map((show, index) => (<div key={index} className="p-4 border border-gray-100 rounded-xl">
                    ...
                    <div className="flex justify-start gap-4 font-light">
                      ...
                      <span>
                        ...
                        {new Date(show.date || "").toLocaleTimeString("fr-FR", {
                          hour: "2-digit",
                          minute: "2-digit"
                        })}
                      </span>
                    </div>
                    <button onClick={() => router.push(show.linkShow || "")} className="mt-4 rounded-lg w-full bg-gray-100 h-10 text-sm font-light">
                      Réserver
                    </button>
                  </div>))}
