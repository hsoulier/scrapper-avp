...
const init = async () => {
  ...
  for (const title of titles) {
    ...
    for (const preview of previews) {
      ...
      for (const detail in el.dataset) {
        ...
        details[detail] = el?.getAttribute(`data-${detail}`);
      }
      previewsList.push({
        ...title,
        ...details,
        previewType: preview?.textContent?.trim(),
      });
    }
  }
  ...
  writeFileSync(
    "./data/ugc-previews.json",
    JSON.stringify(previewsList),
    "utf-8"
  );
};
